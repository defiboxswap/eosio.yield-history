...
class HistoryTrace {
  ...
  async receivedBlock(response, block, traces, deltas) {
    ...
    // if (!(block_num % 10000)) {
    //   await conn.query('delete from dex_blocks where block_num < ?', [block_num - 10000]);
    //   await conn.query('delete from dex_events where block_num < ?', [block_num - 10000]);
    //   await conn.query('delete from dex_actions where block_num < ?', [block_num - 10000]);
    // }
    // if (block_num >= 500)
    //     process.exit(1);
    try {
